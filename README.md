...
> **Corrección de carga de presets (agosto 2026):** se solucionó un problema por el que, al cargar ciertos archivos `.maxpresets`, alguna gráfica de frecuencia o de amplitud podía aparecer **vacía aunque el archivo sí tuviera esos datos guardados**. Pasaba por dos causas internas, ya resueltas:
> - Algunos archivos que en algún momento pasaron por la actualización automática desde una versión antigua del patch (previa a tener varios canales) quedaban con un "resto" duplicado y vacío de esa misma gráfica. Al recargar el archivo, ese resto vacío tapaba a los datos reales.
> - El patch necesita saber si un archivo es "moderno" (con varios canales guardados) o "antiguo" (un solo canal). Antes se apoyaba en una marca interna que, en la práctica, nunca llegaba a guardarse al pulsar **Guardar**; ahora lo deduce mirando cuántos canales traen realmente datos dentro del propio archivo, así que la detección nunca falla.
>
> No hay que hacer nada especial: el arreglo es automático y afecta solo a la lectura del archivo. Cualquier `.maxpresets` guardado con el botón **Guardar** actual conserva siempre sus cinco canales intactos al volver a cargarlo.
...
**Cuatro trampas de Max que costaron caras aquí** (si tocas este código, léelas antes):
- **`buffer~` no sirve para cortar por muestra: maneja los tiempos en milisegundos con precisión de 32 bits.** Un float de 32 bits da unas 7 cifras significativas, así que a los 10 minutos de toma (~600 000 ms) el error de representación ya pasa de **una muestra**. Medido en una toma real de 13 minutos: los trozos empezaban 0 o 1 muestra antes de lo pedido *según su posición en el maestro*, y a la mitad les faltaba la última muestra, que salía en 0. No hay margen de redondeo que lo arregle, porque el error crece con la magnitud del número. Encima, `buffer~` guarda el audio en float de 32 bits, así que el viaje int24 → float32 → int24 cambiaba algún bit menos significativo. Por eso el troceo se sacó de Max: ahora los cortes van **en muestras (enteros)** a `rec_split_node.js`, que copia los bytes del maestro — exacto por construcción e idéntico bit a bit. Regla general: **a `buffer~` no le pidas precisión de muestra en milisegundos.**
...
Los cortes se calculan en `rec_split.js`; la carpeta la crea y el maestro lo trocea `rec_split_node.js` (Node for Max) con `aiff_slice.js`. El parcheo del `.maxpat` está en `scratch/patch_rec_split.js`, y las comprobaciones sin Max en `scratch/test_rec_split.js` (21, entre ellas que la grabación arranca sin esperar a node y que a node se le mandan muestras enteras, nunca milisegundos) y `scratch/test_aiff_slice.js` (trocea de verdad y compara byte a byte contra el original, incluida la última muestra; si encuentra un maestro real lo usa también).

> **Al tocar `rec_split_node.js` hay que reiniciar el nodo.** El `js` se recarga solo (`autowatch 1`), pero `node.script` no: hay que mandarle `script restart` o volver a abrir el patch. Y si `node.script` no estuviera corriendo, el troceo no se hace — el maestro queda íntegro y el aviso sale en la consola de Max.
